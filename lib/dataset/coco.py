...
class CocoDataset(AutoDriveDataset):
    ...
    def _get_db(self):
        ...
        for mask in tqdm(list(self.mask_list)):
            ...
            lane_path = mask_path.replace(str(self.mask_root), str(self.lane_root)).replace(".png", ".jpg")
            with open(label_path, 'r') as f:
                label = json.load(f)
            data = label['frames'][0]["labels"]
            # data = self.filter_data(data)
            gt = np.zeros((len(data), 5))
            # width=label['frames'][0]['size']["width"]
            # height=label['frames'][0]['size']["height"]
